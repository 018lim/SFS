...
- **`op`이 정상이어도 배당/자사주는 따로 실패할 수 있다.** 현금흐름표는 손익과 달리 "연간 − Q3누적" 같은 우회로가 없다(3개월 단독 보고서 자체가 없이 언제나 누적이라서). `sfs_utils.get_discrete_cf`는 무조건 `YTD(q) - YTD(q-1)`이라, 중간 분기 하나가 API 공백이면(금융업 2023Q1~Q2 등) 그 뒤 정상 분기(2023Q3)까지 `None`으로 끌려간다 — `op`은 그 분기 자체 보고서만 보므로 멀쩡히 나오는데 배당·자사주만 빠지는 식이다. `is_api_gap`이 `op`만 보므로 이 경우 원문 폴백도 안 걸린다. `sfs_step2_data_builder.discrete_cf`가 직접 차감이 실패하면 **이미 저장된 앞 분기들의 discrete 값 합**으로 대신 구해 우회한다(Tier-1이 그 해 1분기부터 받으므로 앞 분기 값은 이미 있다).
